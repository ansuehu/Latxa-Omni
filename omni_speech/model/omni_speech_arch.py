...
class OmniSpeechMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_speech_and_text(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            # Safe Fallback: If no speech tokens are present but we have speech features
            if num_speech == 0:
                if cur_speech_idx < len(speech_features):
                    cur_speech_features = speech_features[cur_speech_idx]
                    cur_speech_idx += 1
                else:
                    cur_speech_features = speech_features[-1] if len(speech_features) > 0 else None
                
                cur_input_embeds_1 = self.get_model().embed_tokens(cur_input_ids)
                if cur_speech_features is not None:
                    cur_input_embeds = torch.cat([cur_input_embeds_1, cur_speech_features[0:0]], dim=0)
                else:
            ...
            for i in range(num_speech + 1):
                ...
                cur_new_labels.append(cur_labels_nospeech[i])
                if i < num_speech:
                    # CRITICAL FIX: Prevent IndexError if speech_features runs short
                    if cur_speech_idx < len(speech_features):
                        cur_speech_features = speech_features[cur_speech_idx]
                        cur_speech_idx += 1
                    else:
                        # Fallback to the last available audio track if history is mismatched
                        cur_speech_features = speech_features[-1] 
