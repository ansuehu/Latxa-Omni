...
class ModelWorker:
    ...
    # --- NEW OMNI-2 INSPIRED HISTORY LOGIC ---
    def load_speech_from_path(self, path):
    ...
    def generate_stream(self, params):
        ...
        if "history" in params:
            ...
            # Fallback for single-turn testing
            prompt = params["prompt"]
            ori_prompt = prompt
            audio = params.get("audio", None)
            ...
            if audio is not None and len(audio) > 0:
                speech = load_speech(audio, self.input_type, self.mel_size, self.model.config.speech_normalize)
                speech_lengths = torch.LongTensor([speech.shape[0]]).unsqueeze(0).to(self.device)
                speech_tensor = speech.unsqueeze(0).to(self.device, dtype=torch.float16)
