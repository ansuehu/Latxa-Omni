...
def build_demo(embed_mode, vocoder, cur_dir=None, concurrency_count=10):
    ...
    # 1. Apply a cleaner, more modern Gradio theme
    ...
    with gr.Blocks(title="Latxa-Omni Speech Interface", theme=custom_theme, css=block_css) as demo:
        ...
        if not embed_mode:
            ...
            <div style="text-align: center; margin-bottom: 20px;">
                <h1>🗣️ Latxa-Omni</h1>
                <h3 style="color: gray;">Open Modular Speech-to-Speech Dialogue System</h3>
            </div>
            """)
        ...
        with gr.Row():
            ...
            with gr.Column(scale=3):
                chatbot = gr.Chatbot(
                    ...
                    avatar_images=(None, "https://api.dicebear.com/7.x/bottts/svg?seed=Latxa") # Optional bot avatar
                )
            
            # 3. Side Control Column
            with gr.Column(scale=1):
                ...
                # Focused Audio Input: Styled specifically for voice
                audio_input_box = gr.Audio(
                    sources=["microphone"], 
                    type="filepath", 
                    label="Speak to Latxa",
                ...
                # 4. Auto-playing Output: Crucial for speech-to-speech feel
                audio_output_box = gr.Audio(
                    label="Response Audio", 
                    show_download_button=False, 
                    autoplay=True, # Automatically plays the generated response
                    visible=True 
                )
                
                clear_btn = gr.Button(value="🗑️ Clear Conversation", variant="secondary")

        # Hide technical parameters by default to keep the UI clean
        ...
        # 5. AUTO-SEND ON RECORDING STOP
        # Trigger the bot immediately when the user stops the microphone
        audio_input_box.stop_recording(
            http_bot,
            inputs=[
                history_state, 
                audio_input_box, 
