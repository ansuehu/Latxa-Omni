#!/usr/bin/env python
"""Convert a Whisper model in Hugging Face format to OpenAI format.
...
>> audio_path = "https://huggingface.co/datasets/sanchit-gandhi/librispeech_long/resolve/main/audio.wav"  # pylint: disable=line-too-long # noqa: E501
...
def reverse_rename_keys(s_dict: dict) -> dict:
    """Rename the keys back from Hugging Face to OpenAI Whisper format.

    By using this function on an HF model's state_dict, we should get the names
    in the format expected by Whisper.
...
def make_emb_from_linear(linear: nn.Linear) -> nn.Embedding:
    """Convert a linear layer's weights into an embedding layer.

    The linear layer's `in_features` dimension corresponds to the vocabulary
    size and its `out_features` dimension
...
def extract_dims_from_hf(config: WhisperConfig) -> dict:
    """Extract necessary dimensions from Hugging Face's WhisperConfig.
...
def convert_tfms_to_openai_whisper(hf_model_path: str, whisper_dump_path: str):
    """Convert a Whisper model from the Hugging Face to the OpenAI format.
    ...
    Args:
        ...
        whisper_dump_path (`str`):
            Destination path where the converted model in Whisper/OpenAI format
            will be saved.
